...
$("#bookingForm").addEventListener("submit", async e => {   e.preventDefault();    const form = e.currentTarget;    if (!form.checkValidity()) {     form.reportValidity();     return;   }    const submitBtn = form.querySelector('button[type="submit"]');   const originalText = submitBtn.innerHTML;    submitBtn.disabled = true;   submitBtn.innerHTML = "Sending...";    const formData = new FormData();    formData.append("entry.274821221", form.name.value);   formData.append("entry.610284649", form.email.value);   formData.append("entry.1010500656", form.phone.value);   formData.append("entry.1454081700", form.event.value);   formData.append("entry.693021019", form.date.value);    // Location   formData.append("entry.1658684155", form.location.value);    // Message   formData.append("entry.756000062", form.message.value);    try {     await fetch(       "https://docs.google.com/forms/d/e/1FAIpQLScu743pB-gsipTX6S2FFkc_pl3oZ49XEke3ECJ6jSifCkD5-Q/formResponse",       {         method: "POST",         mode: "no-cors",         body: formData       }     );      alert("Thank you! Your booking enquiry has been sent successfully.");      form.reset();    } catch (error) {      console.error(error);      alert(       "Something went wrong. Please try again or contact us on WhatsApp."     );    } finally {      submitBtn.disabled = false;     submitBtn.innerHTML = originalText;    } });
